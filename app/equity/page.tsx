...
import { cagrPercent } from "@/lib/returns";
...
export default async function EquityPage({ searchParams }: { searchParams?: Promise<{ warnings?: string }> }) {
  ...
  const firstTxnDate = txns.map((t) => t.txn_date).filter(Boolean).sort()[0];
  const actualCagr = firstTxnDate ? cagrPercent(totalInvested, totalCurrent, firstTxnDate, dayjs().format("YYYY-MM-DD")) : null;
  ...
  return <div className="space-y-5"><h1 className="text-2xl font-bold">Equity & Mutual Funds</h1>
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-4"><div className="ta-card p-4"><p className="text-xs text-slate-500">Total Invested</p><p className="text-xl font-bold">{formatCurrency(totalInvested)}</p></div><div className="ta-card p-4"><p className="text-xs text-slate-500">Current Value</p><p className="text-xl font-bold">{formatCurrency(totalCurrent)}</p></div><div className="ta-card p-4"><p className="text-xs text-slate-500">Unrealized P/L</p><p className="text-xl font-bold">{formatCurrency(totalCurrent-totalInvested)}</p></div><div className="ta-card p-4"><p className="text-xs text-slate-500">Actual CAGR</p><p className="text-xl font-bold">{Number.isFinite(actualCagr) ? `${(actualCagr as number).toFixed(2)}%` : "-"}</p></div></div>
