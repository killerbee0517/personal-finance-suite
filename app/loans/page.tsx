...
export default async function LoansPage({ searchParams }: { searchParams?: Promise<{ filter?: string }> }) {
  ...
  return <div className="space-y-5"><div className="flex items-center justify-between"><h1 className="text-3xl font-bold tracking-tight">Loan Tracker</h1><Link className="ta-btn" href="/loans/new">Add Loan</Link></div>
    ...
    <div className="ta-card overflow-hidden"><div className="overflow-x-auto"><table className="ta-table min-w-full"><thead><tr><th>Lender</th><th>Start Date</th><th>Outstanding</th><th>Rate</th><th>Due</th><th>Linked FDs</th><th /></tr></thead><tbody>{loans.map((loan)=><tr key={loan.id}><td>{loan.bank_name} | {loan.loan_type}</td><td>{loan.start_date}</td><td>{formatCurrency(loan.outstanding_principal)}</td><td>{loan.interest_rate}%</td><td>{loan.end_date}</td><td>{links.filter((l)=>l.loan_id===loan.id).length}</td><td><Link href={`/loans/${loan.id}`} className="ta-btn-outline">View</Link></td></tr>)}</tbody></table></div></div>
  </div>;
}
